...
export function HotfixPanel({ isOpen, onClose }: HotfixPanelProps) {
  ...
  const loadHotfixStatus = async () => {
    try {
      const status = await gitService.getHotfixStatus()
  ...
  const handleAbortHotfix = async () => {
    ...
    if (!confirm(t('hotfix.confirmAbort', '确定要取消此 Hotfix 吗？这将删除 hotfix 分支和 worktree。'))) {
      return
    }
  ...
  const isInProgress = hotfixStatus?.status === 'inProgress'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* 面板 */}
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[80vh] overflow-auto">
        {/* 头部 */}
