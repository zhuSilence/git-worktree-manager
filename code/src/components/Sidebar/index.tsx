...
export function Sidebar({ onRepoSelect }: SidebarProps) {
  ...
  const handleRepoClick = (repo: RepositoryInfo) => {
    if (repo.isPathValid === false) {
      // 无效仓库，提示用户是否移除
      if (confirm(t('sidebar.confirmRemoveRepo', { name: repo.name }))) {
    ...
    return (
      <aside className="w-12 h-full bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col items-center py-2 transition-all duration-200">
        ...
        <button
          ...
          title={t('sidebar.expandRepoList')}
        >
        ...
        <div className="flex-1 overflow-y-auto flex flex-col items-center gap-1.5 w-full px-1">
          {repositories.map((repo) => (
            <button
              ...
              title={repo.isPathValid === false ? t('sidebar.pathInvalidTitle', { name: repo.name }) : repo.name}
            >
              {repo.isPathValid === false ? (
    ...
    <aside className="w-64 h-full bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col transition-all duration-200">
      ...
      <div className="p-3 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={toggleCollapsed}
              className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 rounded"
              title={t('sidebar.collapseRepoList')}
            >
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400">
          {t('sidebar.repoCount', { count: repositories.length })}
          {invalidCount > 0 && (
            <span className="text-red-500 ml-1">({t('sidebar.invalidCount', { count: invalidCount })})</span>
          )}
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {repositories.length === 0 ? (
          <div className="p-4 text-center text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm">{t('sidebar.noRepos')}</p>
            <button
              onClick={handleAddRepo}
              ...
              >
                ...
                {activeRepoId === repo.id && repo.isPathValid !== false && (
                  <div className="mt-2 flex items-center text-xs text-green-600 dark:text-green-400">
                    ...
                    <span>{t('sidebar.currentSelected')}</span>
                  </div>
                )}
              </div>
            ))}
      ...
      <div className="p-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
        {invalidCount > 0 && (
          <button
            onClick={() => {
              if (confirm(t('sidebar.confirmRemoveInvalid', { count: invalidCount }))) {
          ...
          >
            <AlertTriangle className="w-4 h-4" />
            {t('sidebar.removeInvalidRepos', { count: invalidCount })}
          </button>
        )}
        <button
          onClick={handleAddRepo}
